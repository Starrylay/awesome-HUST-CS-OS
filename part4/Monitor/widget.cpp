...
void Widget::createCanvas()
{
    ...
    }void Widget::on_createfile_clicked()
    {
        ui->editfiletext->clear();
    }

    void Widget::on_openfile_clicked()
    {
        QString fileName=QFileDialog::getOpenFileName(this, tr("打开一个文件"),
                         "/home/chengleishen/OSdesign",
                         "*.txt");
        if(fileName.isEmpty())
        {
        ...
        else
        {
            ...
            ui->editfiletext->setText(QString(ba));
        }
    ...
    {
        QString fileName=QFileDialog::getSaveFileName(this, tr("选择一个文件"),
                         "/home/chengleishen/OSdesign",
                         "*.txt");
        if(fileName.isEmpty())
        {
        ...
        else
        {
            ...
            QByteArray ba;
            ba.append(ui->editfiletext->toPlainText());
            file.write(ba);
            file.close();
        }
    }
...
void Widget::setSysInfo()
{
    ...
    /*显示个人说明*/
    ui->label_5->setText("开发者：CS1903 沈承磊\n\nE-mail：896600038@qq.com\n");
...
void Widget::updateCurTime()
{
    ...
    sprintf(time,"%d.%d.%d %02d:%02d:%02d",ptm->tm_year+1900,
            ptm->tm_mon+1,ptm->tm_mday,ptm->tm_hour,ptm->tm_min,ptm->tm_sec);
    ui->label_3->setText("北京时间："+QString(time));
...
void Widget::updateSwapUsg()//调整为磁盘IO操作时间百分比
{
    /* 将曲线向前移动一个单位 */
    ...
    for(int i=0;i<12;i++)
    {
         ...
         usetimenew+=templist[21].toInt();
    }
//     templist = strList[12].split(" ");
//     usetimenew=templist[20].toInt();
    ...
    double IO_V = 100*(usetimenew-usetimeold)/500;
    usetimeold = usetimenew;
    points.append(QPointF(oldPoints.count()-1 ,IO_V));
    swap_series->replace(points);
    QString usetimenew_str = QString::number(usetimenew);
    ui->label_14->setText( "   I/O:\n  " + usetimenew_str + "ms" );
